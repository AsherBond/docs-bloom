...
[NOTE]
====
When using Bloom in the Aura console, it is currently not possible to select a different database for your Perspective from the Perspective Gallery.
====
...
The list of property keys include their data type and an option to exclude them from the visualization.
By default, any properties indexed by a *vector* index are automatically excluded.

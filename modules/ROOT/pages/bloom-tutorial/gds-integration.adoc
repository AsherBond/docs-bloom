...
Alternatively, if you connect to a *Neo4j Aura instance*, GDS algorithms can run either in a separate compute environment via the serverless option, or in the same AuraDB instance as the database using the Graph Analytics plugin, sharing resources.
This is selected when you create an AuraDB instance in the Neo4j Aura console.
For more information, see link:https://neo4j.com/docs/aura/getting-started/create-instance/[Neo4j Aura -> Create an instance].

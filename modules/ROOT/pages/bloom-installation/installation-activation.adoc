...
However, in Desktop 2.0, Bloom is available in the Desktop Studio, and in earlier versions, Bloom is available as a graph app.
The Bloom client is enabled and ready to use in all versions of Desktop.

In Desktop 2.0, you find Bloom in the *Studio* section on the sidebar.
See link:https://neo4j.com/docs/desktop/2.0/operations/connections/#_connecting_tools[Connecting tools] for more information on how to use Bloom in Desktop 2.0.
...
Access to a remote Neo4j database and features used for collaboration, such as persistent storage and security, require the xref:bloom-installation/installation-activation.adoc#installation-bloom-server [Bloom server] component.
====
...
See step 5 *Setup users/roles in the database* further on, see also link:{operations-manual-base-uri}current/authentication-authorization/[Operations Manual -> Authentication and authorization] for more information on users and roles.
====
...
For example, if you are using Neo4j 2025.10 and Bloom 2.28.0, use `bloom-plugin-6.x-2.28.0.jar`.
+
...
See link:{operations-manual-base-uri}current/authentication-authorization/[Operations Manual -> Authentication and Authorization] for more info on roles in Neo4j.
+

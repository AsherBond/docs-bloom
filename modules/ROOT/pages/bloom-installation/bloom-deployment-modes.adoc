...
As mentioned in xref:bloom-installation/about-bloom.adoc/#_neo4j_desktop_hosted[Neo4j Desktop hosted], Bloom is available as *Explore* in Neo4j Desktop.
The Explore client app in Neo4j Desktop can be used independent of the Bloom server component.
The client makes a direct connection to your Neo4j graph database and runs queries directly using this connection.

But, Bloom/Explore is significantly more useful when both the client and server components are used together.
...
== Explore app in Neo4j Desktop

*Without the server plugin*

In this mode, the built-in Explore tool can connect to a Neo4j database locally created in Desktop only.
Without the server plugin, the client stores Perspective definition in a local storage allocated to the app.
This is effectively a single user mode for Explore and may be used in an evaluation or proof of concept to try Explore against your graph data.

[WARNING]
====
Since local storage is not considered persistent in the long term, consider exporting Perspectives if you wish to preserve them and to avoid accidental deletion or overwrites
...
In this mode, the server plugin component is installed on the Neo4j database.
The database can be locally created in Desktop, or in a remote server instance or cluster.
When the Explore client connects to the Neo4j database, it checks for the presence of the Bloom server plugin.
If found, the Explore client relies on the server to provide storage and user authorization capabilities.


image::bloom-app-in-neo4j-desktop.png[width=500,align="center"]

Even though the server plugin can package and host the Bloom client, you may prefer to use your own web server and xref::/bloom-installation/advanced-installation.adoc#bloom-web-app-on-separate-server[host the Bloom client separately] from the Neo4j database server.
